...
该独立测试补丁用于验证 iOS 战斗刷怪阶段的原生空函数指针崩溃。两份 IPS 均定位到
...
第六版测试包的 `CFBundleVersion` 为 `1.8.46`，Mach-O UUID 为
`064f8e49-fdd9-532f-8445-439ab9f71e6c`。如果仍然闪退，新 IPS 必须出现这两个标记，
否则不能视为第六版实际运行。
...
的空实现调用，不掩盖其他地址的崩溃，也不替代后续真机长时间回归。静态检查只能验证
指令、具体类型/字段数据路径和控制流，是否适配目标设备上的 AIR 运行时仍须真机验证。
回滚时重新安装原最终 IPA 即可。
