...
| `BattleNotifyMessage.LevelNext(1)` | Notify | 双 Boss 切换；保留当前战斗连接并重置下一阶段的 SceneReady 屏障 |
| `BattleNotifyMessage.Finalize(2)` | Notify | → `Finalized(2)` |
| `BattleNotifyMessage.Measurement(3)` | Notify | → 回显 `Measurement(3)` |
| `BattleNotifyMessage.LineSpeedWarning(4)` | Notify | 线路延迟警告 |
| `BattleNotifyMessage.Heartbeat(5)` | Notify | → 回显 `Measurement(3)` |
