...
| 2630023 | 女王拉芙 | 151006 |
| 1310052 | 结实假人·水（Practice 25） | 131005 |

任务主表的 `quest_kind=11` 会由客户端解析为 Practice，且 `row[12]="25"`
就是目标 Quest ID。旧实现误用了 `96`，因此真实结算的 Practice 25 无法触发任务；
同名的 Practice 92 不属于该任务目标。
