...
- `planEquipmentEnhancementPurchase()` 校验当前强化等级、觉醒等级与购买数量，并把合法购买推进到当前材料阶段的上限；
- 私服明确保留“阶段直达”福利：当前阶段只收取 1 份该阶段标价材料，即使本次跨过多个强化等级；福利不能跨越下一条商店商品，后续关键材料阶段仍须分别购买；
- 购买商品必须是同一商店分类、装备与强化组中的当前阶段，不能通过提交后续 `shop_item_id` 越过中间材料；
...
客户端展示的“阶段”与数据库中的 `enhancementLevel` 是同一条状态链的不同表现，不能把商店阶段号直接当作最终装备等级。部署修复不会回退玩家已经取得的强化等级；既有进度会从下一条合法阶段继续。

解放者（5010070）与终结者（5020043）各自使用 13 个阶段，阶段上限依次为 `1 / 12 / 23 / 34 / 45 / 56 / 69 / 70 / 77 / 84 / 91 / 98 / 99`。每个阶段均标价皮那水晶（40313）×1；两把武器的最终强化等级上限仍为 99，其余武器的阶段配置不随本调整改变。
