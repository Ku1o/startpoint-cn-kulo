...
function ensureSchemaColumn(database, key) {
    ...
    const columns = database.prepare(`PRAGMA table_info("${schema.table}")`).all();
    if (columns.some(column => column.name === schema.column))
        return false;
    database.prepare(`ALTER TABLE "${schema.table}" ADD COLUMN "${schema.column}" ${schema.definition}`).run();
    return true;
}
exports.ensureSchemaColumn = ensureSchemaColumn;
