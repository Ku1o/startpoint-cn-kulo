...
function hasPrimaryKey(columns, expected) {
 ...
 */
function repairCategoryMissionTables(database) {
    ...
    const stageShapeValid = stageColumns.length === 0 || (["category", "id", "status", "player_id", "mission_id"]
        .every(name => stageColumns.some(column => column.name === name))
        && hasPrimaryKey(stageColumns, ["category", "id", "mission_id", "player_id"]));
    if (missionShapeValid && stageShapeValid)
        return;
    ...
    try {
        database.transaction(() => {
            ...
            const insertMission = database.prepare(`
                ...
                VALUES (?, ?, ?, ?)
            `);
            const normalizedMissionRows = new Map();
            ...
            const restoredMissionKeys = new Set();
            ...
            const insertStage = database.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?)
            `);
            const normalizedStageRows = new Map();
            ...
            for (const row of normalizedStageRows.values()) {
                if (!restoredMissionKeys.has(`${row.category}:${row.mission_id}:${row.player_id}`))
                    continue;
    ...
    console.log(`[DB] repaired category mission schema: missions=${missionRows.length} stages=${stageRows.length}`);
}
function init(database, exists) {
    ...
    try {
        database.prepare(`ALTER TABLE accounts ADD COLUMN admin_note TEXT DEFAULT NULL`).run();
    }
    catch ( /* column already exists */_a) { /* column already exists */ }
    try {
        database.prepare(`ALTER TABLE accounts ADD COLUMN takeover_password TEXT DEFAULT NULL`).run();
    }
    catch ( /* column already exists */_b) { /* column already exists */ }
    try {
        database.prepare(`ALTER TABLE accounts ADD COLUMN takeover_udid TEXT DEFAULT NULL`).run();
    }
    ...
    try {
        database.prepare(`ALTER TABLE players ADD COLUMN time_offset INTEGER DEFAULT NULL`).run();
    }
    ...
    try {
        database.prepare(`ALTER TABLE players_quest_progress ADD COLUMN host_finished INTEGER NOT NULL DEFAULT 0`).run();
    }
    ...
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.single_play_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.single_clear_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.multi_play_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.multi_clear_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.multi_host_clear_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.multi_guest_clear_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.dash_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.power_flip_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_periodic_snapshots.login_days");
    ...
    (0, schema_1.ensureSchemaColumn)(database, "players_mission_battle_counters.single_rank_ss_count");
    ...
    catch ( /* column already exists */_z) { /* column already exists */ }
    try {
    ...
    (0, schema_1.ensureSchemaColumn)(database, "players_quest_progress.leader_character_id");
    (0, schema_1.ensureSchemaColumn)(database, "players_quest_progress.multi_clear_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_quest_progress.unlocked");
    (0, schema_1.ensureSchemaColumn)(database, "players_quest_progress.s_plus_reward_received");
    ...
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.total_equipment_equip_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.total_unison_set_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.total_party_character_set_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.total_injected_exp_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.total_gacha_campaign_count");
    (0, schema_1.ensureSchemaColumn)(database, "players_active_mission_counters.practice_quest_challenge_count");
    ...
    try {
        database.prepare(`ALTER TABLE players_multi_special_exchange_campaigns ADD COLUMN ticket_item_id INTEGER`).run();
    }
    catch ( /* column already exists */_1) { /* column already exists */ }
    ...
    // version is older.
    try {
        database.prepare(`ALTER TABLE raid_event_global_state ADD COLUMN weighted_kill_count INTEGER NOT NULL DEFAULT 0`).run();
    }
    catch ( /* column already exists */_2) { /* column already exists */ }
    try {
        database.prepare(`ALTER TABLE raid_event_global_state ADD COLUMN calculation_version INTEGER NOT NULL DEFAULT 1`).run();
    }
    catch ( /* column already exists */_3) { /* column already exists */ }
