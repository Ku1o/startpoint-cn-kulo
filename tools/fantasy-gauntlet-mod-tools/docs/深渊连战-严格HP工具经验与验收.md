...
## 6. 运行时语义门禁：不能只证明数据可解析

本轮三个事故的共同原因，是旧流程验证了“数据类型、资源存在、表结构可读”，却没有
证明客户端运行时如何解释这些数据。后续构建必须同时执行以下门禁：

- ActionDSL 修改先按官方命令签名标注每个参数的位置和语义，再声明本次允许修改的参数位。
  例如 `AddSkillPoint` 的参数 1 是对象 ID、参数 2 才是数值；二者都是整数或数字时，
  单靠类型检查发现不了把对象 `20` 改成 `15` 的错误。写回后还要重新解析并校验整棵 DSL。
- Boss 移植到随机场地时，必须调用 ActionDSL/ESDL 推导出的正式场地兼容性检查，覆盖
  `p0/p1/p2` 自定义坐标、funnel 数量、活动层、slot/group 拓扑等依赖。Boss 槽数量和
  历史白名单只能用于筛选，不能作为可运行证明；证据不足时回退原生场地或使构建失败。
- Boss 关封面必须从实际 Boss 身份反查官方任务大图。先匹配完全相同的 Boss ID 组合；
  官方 single/multi/tower 别名 ID 不同时，才以“官方显示名 + Boss 模型根”匹配同一视觉
  Boss。`floor_host_quest` 只说明某场地曾属于一层，不能证明图片中的 Boss，禁止作为
  最终封面证据。

每次真机暴露的新运行时语义错误，都必须同时补一个能复现旧错误的回归测试；只修当前
生成结果、没有升级门禁的改动不算工具修复完成。
