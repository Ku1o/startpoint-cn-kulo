...
`29/29 absolute` 只证明本次内存构建的 HP 与静态闭包满足契约，不能证明 Boss AI、阶段时序、封面、资源加载或整塔结算已经通过。没有真机条件时必须一直保留“未真机验证”标记。

已知不在本工具当前修复范围内的事项：关卡封面与实际 Boss 不符。该问题暂不作为严格 HP 构建失败条件，也不能在报告中写成已解决。
