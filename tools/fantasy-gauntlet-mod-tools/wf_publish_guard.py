...
def chain_latest_index() -> dict[str, tuple[str, Path, str]]:
    ...
    # 本项目的实际发布链位于 assets/asset-patch/active。它必须最后覆盖
    # 官方 CDN 底座，否则防回退检查会拿很早的官方表和当前待发布表比较，
    # 无法发现后续自定义更新之间的整表回退。
    try:
    ...
    for patch in manifest.get("patches", []):
        ...
        if patch.get("archive"):
            archives.append(str(patch["archive"]))
        archives.extend(str(x) for x in patch.get("chain", []) if x)
        version = str(patch.get("version") or patch.get("to_version") or "active")
        for archive in archives:
            path = ACTIVE_PATCH / Path(archive).name
            if not path.is_file():
                continue
            try:
                names = zipfile.ZipFile(path).namelist()
                ...
                continue
            for name in names:
                relative = name.replace("production/upload/", "")
                index[relative] = (version, path, name)
    return index
...
def main(argv: list[str] | None = None) -> int:
    ...
    store = core.resolve_active_store(TOOL_DIR.parent, profile=profile)
    if args.tables:
        import wf_publish
