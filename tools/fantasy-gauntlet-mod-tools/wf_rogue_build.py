...
def special_boss_kinds() -> dict:
    """专用 Boss 代号到 BossKind 的反查表；进程内缓存。"""
...
def zone_boss_kind_fixer(general_boss: dict, standard_boss: dict):
    """返回 Boss 代号换列时同步校正 BossKind 的回调。"""
    special = special_boss_kinds()

    def kind_of(code: str, current_kind) -> int | None:
        code = str(code)
        try:
            current = int(str(current_kind).strip())
        except (TypeError, ValueError):
            current = None
        if current is not None:
            if current in (1, 8) and code in general_boss:
                return None
            if current == 0 and code in standard_boss:
                return None
            if special.get(code) == current:
                return None
        if code in general_boss:
            return 1
        if code in standard_boss:
            return 0
...
def _sync_boss_kind(wc: list[str], code_index: int, code: str, kind_of) -> None:
    """同步 Boss 代号左邻的 BossKind 列。"""
    if kind_of is None:
        return
    kind_index = code_index - 1
    if kind_index < 0 or kind_index >= len(wc):
        return
    wanted = kind_of(code, wc[kind_index])
    if wanted is not None and str(wanted) != str(wc[kind_index]):
        wc[kind_index] = str(wanted)
...
def main() -> int:
    ...
    def gimmick_field(orig_field: str, r: int, panels: bool = False,
        ...
        for wk, wrow in zn.items():
            ...
            if boss_swap:
                apply_boss_swap(wc, boss_swap[0], boss_swap[1], kind_of=kind_fixer)
    ...
    def mix_pick(r: int, pin_terrain: str | None = None,
        ...
        fc = cells(frow)
        zkey, fkey = f"mod_rogue_z{r}", f"mod_rogue_f{r}"
        zone_t[zkey] = swap_zone_bosses(
            zone_t[fc[2]], sbosses, kind_of=kind_fixer
        )
        nf = list(fc)
        nf[2] = zkey
        fd_t[fkey] = join(nf, isinstance(frow, (bytes, bytearray)))
