...
SOURCE_ASSET_DIR = MOD_DIR / "assets" / "abyss-equipment"
DEFAULT_UPLIFT_MULTIPLIER = (3, 2)
# Keep the Fantasy Gauntlet's established level curve: level 1 starts at
# half of the audited/up-lifted level-5 amplitude, with client interpolation
# providing the intermediate levels.
...
def build_soul_leaf(
    ...
    for slot, effect in enumerate(spec.effects, start=1):
        ...
        resolution = resolve_effect_strength(spec, effect)
        strength = resolution.value
        if strength is not None:
            row[49] = str(strength)
            row[48] = (
                str(round(strength * WEAPON_LEVEL1_RATIO))
                if resolution.rule.mode == "scale"
                else str(strength)
            )
...
def load_json(name: str):
    with open(core.resolve_server_dir() / "assets" / name, encoding="utf-8") as fh:
        return json.load(fh)


def save_json(name: str, data) -> None:
    with open(core.resolve_server_dir() / "assets" / name, "w", encoding="utf-8") as fh:
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="wf-abyss-release-snapshot-") as temp:
            ...
            command = [
                sys.executable,
                str(MOD_DIR / "wf_publish.py"),
                "--tables",
                publish_tables,
                "--snapshot",
                str(snapshot_path),
            ]
            subprocess.run(command, cwd=server_root, check=True)
