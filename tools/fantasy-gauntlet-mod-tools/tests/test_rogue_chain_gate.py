...
class StatNormalizeCase(unittest.TestCase):
    ...
    def test_growth_curves_decode(self):
        ...
        钉住三条下载 hp 曲线在 lv100 的值；APK 内置默认曲线
        可以合并进来，不应再用“总数必须恰好为 3”拒绝它们。"""
        c = rb.growth_curves()
        if not c.get("hp"):
            self.skipTest("store 不可用")
        self.assertTrue(
            {"hit_hp_correction_non_element", "hit_hp_boss", "hit_hp_funnel"}
            <= set(c["hp"])
        )
    ...
    def test_floor_native_hp_keeps_two_real_instances_with_the_same_code(self):
        ...
        with self.subTest("Orochi expands one parent plus eight ordered head instances"):
            ...
            for variant in ("single", "multi", "multi_plus"):
                ...
                self.assertAlmostEqual(
                    expanded.total_hp,
                    math.fsum(member.raw_hp for member in expanded.members),
                    places=5,
                )
        ...
        with self.subTest("duplicate head code is accumulated by occurrence"):
            ...
            original = expanded_by_variant["single"]
            expected_duplicate = (
                original.total_hp
                - original.members[-1].raw_hp
                + original.members[2].raw_hp
            )
            self.assertAlmostEqual(
                expanded.total_hp, expected_duplicate, places=5
            )
...
class BossLevelHpScalingCase(unittest.TestCase):
    ...
    def test_fix_hp_clone_changes_only_c5_and_preserves_c6_and_source(self):
        ...
        with self.subTest("Orochi clone stages and commits all nine entities together"):
            ...
            original_expanded = rb.expand_bundle_hp_members(bundle, 100, tables)
            self.assertTrue(original_expanded.ok, original_expanded)
            ...
            self.assertAlmostEqual(
                result.expanded.total_hp,
                original_expanded.total_hp * 3.0,
                places=5,
            )
...
class TowerHpTargetCase(unittest.TestCase):
    ...
    def test_deep_hp_anchor_expansion_is_general_c2_scalable(self):
        ...
        for r, field in zip(range(25, 31), fields):
            ...
            resolved.append((r, field, bosses))
        for r, field, bosses in resolved:
    ...
    def test_native_hp_combines_general_formula_and_standard_esdl(self):
        try:
            ...
            self.skipTest("store 不可用")
        # 全局 Boss HP 可以是官方 ×1，也可以由显式增强开关切成 ×4；本测试只证明
        # general 与 standard 两条独立通道会逐实例相加，不把环境状态写死进断言。
        self.assertGreater(got["components"][0]["native_hp"], 0)
        self.assertEqual(got["components"][1]["native_hp"], 385_000_000.0)
        self.assertEqual(
            got["native_hp"],
            math.fsum(component["native_hp"] for component in got["components"]),
        )
        self.assertTrue(got["verified"])
        self.assertIn(got["components"][0]["evidence_kind"], {"absolute", "proxy"})
        self.assertEqual(got["components"][1]["evidence_kind"], "absolute")
        self.assertEqual(
            got["absolute_verified"],
            got["components"][0]["evidence_kind"] == "absolute",
        )
...
class TaskCDryRunCase(unittest.TestCase):
    ...
    def test_canonical_30_floor_dry_run_hits_all_task_c_gates(self):
        ...
        by_round_line = {
            ...
            if (match := re.match(r"\s+第(\d+)战 ", line))
        }
